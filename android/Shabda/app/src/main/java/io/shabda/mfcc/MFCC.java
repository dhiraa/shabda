...
public class MFCC {
	...
	private final static int       n_mfcc       		= 64;//20;
	private final static double    fMin                 = 0.0;
	private final static int       n_fft                = 1024;//2048;
	private final static int       hop_length           = 345;//512;
	private final static int	   n_mels               = 128;

	private final static double    sampleRate           = 44100.0;//16000.0;
	...
	public float[][] processFloat(double[] doubleInputBuffer) {
		final double[][] mfccResult = dctMfcc(doubleInputBuffer);
		return finalshapefloat(mfccResult);
	}

	//MFCC into 1d
	private float[][] finalshapefloat(double[][] mfccSpecTro){
		float[][] finalMfcc = new float[mfccSpecTro.length][mfccSpecTro[0].length];

		for (int i = 0; i < mfccSpecTro.length; i++){
			for (int j = 0; j < mfccSpecTro[0].length; j++){
				finalMfcc[i][j] = (float) mfccSpecTro[i][j];
